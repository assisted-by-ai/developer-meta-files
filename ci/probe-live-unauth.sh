...
if [ "$status" != '200' ]; then
   printf 'skip: rate_limit endpoint HTTP %s; cannot probe.\n' "$status" >&2
   exit 0
fi
remaining="$(jq --raw-output '.resources.core.remaining // 0' /tmp/probe-rl.json)"
if [ "$remaining" -lt 5 ]; then
   printf 'skip: unauth core quota at %s (need >= 5); rate-limit window full.\n' \
      "$remaining" >&2
   exit 0
fi
printf 'unauth core quota: %s\n' "$remaining"
...
if ! grep --quiet --extended-regexp -- '^[0-9]+ repos to process' <<< "$out"; then
   printf 'FAIL: expected "N repos to process" header in output\n' >&2
   exit 1
fi
if ! grep --quiet -- 'DRY-RUN: clone' <<< "$out"; then
   printf 'FAIL: expected at least one "DRY-RUN: clone" line\n' >&2
   exit 1
fi
